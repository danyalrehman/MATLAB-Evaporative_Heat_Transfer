function [derivs] = Filled_Pot_ODEs(t, vars, parameters, other)
...
% Temperature 
T_air = vars(1); % inner chamber, air
T_veges = vars(2); % inner chamber, vegetables 
T_2 = vars(3);  % Inner Pot
T_3 = vars(4); % Sand 
T_4 = vars(5); % Outer Pot
T_5 = vars(6); % Cloth 
V_water = vars(7); % [m3] volume of water in device 

T_roof_ext = vars(8); % Kelvin, external roof surface temp
T_roof_int = vars(9); % Kelvin, internal roof surface temp 
...
N = 0; % Clear skies 
CC = 1 + 0.0224*N - 0.0035*N^2 + 0.00028*N^3; % Cloud cover correlation 
