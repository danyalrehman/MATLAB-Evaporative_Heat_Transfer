function [derivs] = ODE_fxn_Empty_Pot(t, vars, parameters, other)
...
% variables = f(t)
T_1 = vars(1); % inner chamber (air) 
T_2 = vars(2); % clay pot
T_3 = vars(3); % sand 
T_4 = vars(4); % dish (probably plastic)
T_5 = vars(5); % cloth 
...
n = parameters(3, 9); % for sensitivity tests
...
% Weighted average Heat capacity of sand layer 
...
R_21 = log(r2/r1)/(2*pi*k2*H2) + (H2-H1)/(k2*A_1_bottom) + 1/(h1*(A_1_bottom));
...
% Effective surface area for evaporation, based on capillary rise
SA_2_eff = 2*pi*r2*h_cap; % [m2]

% Evaporation Rates
N_evap_2 = SA_2_eff*m_evap_2/MM_H2O; % mol/s
...
dT_2dt = (1/(Cp_2*rho_2*V_2))*(Q_52 + Q_32 - Q_21 + Q_conv_2 + Q_rad_int - Q_evap_2); 
dT_3dt = (1/(Cp_3*rho_3*V_3))*(Q_43 - Q_32 + Q_conv_3 + Q_rad_int - Q_evap_3);
dT_4dt = (1/(Cp_4*rho_4*V_4))*(Q_conv_4 + Q_rad_int - Q_evap_4 ...
    - Q_43); % dish 
dT_5dt = (1/(Cp_5*rho_5*V_5))*(Q_conv_5 + Q_rad_int - Q_evap_5 - Q_51 - Q_52);
